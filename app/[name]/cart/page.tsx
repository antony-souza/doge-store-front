        ...
        <div>
          ...
          {loading ? (
            <div className="text-center mt-4">
              <span className="font-bold text-base">Carregando</span>
            </div>
          ) : (
            <div>
              <div className="pt-3">
                {products.length > 0 ? (
                  products.map((product) => (
                    <div
                      key={product.id}
                      className="p-4 bg-white shadow rounded mb-4 flex gap-2"
                    >
                      <div className="flex gap-2 justify-between">
                        <div>
                          <img
                            ...
                            alt={product.name}
                            className="w-20 h-20 object-cover border-4 rounded shadow"
                          />
                        </div>
                ...
                <div className="flex justify-between mt-4 p-4 bg-white shadow rounded">
                  <div className="flex flex-col items-start">
                    <span className="text-lg font-bold">
                      Total Geral: {total > 0 ? formatPrice(total) : "R$ 0,00"}
                    </span>
                    <span className="text-lg font-bold">
                      Dividido:{" "}
                      {installments > 0
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      ...
                      value={installments}
                      onChange={(e) =>
                        setInstallments(Math.max(Number(e.target.value)))
                      }
                      min={1}
