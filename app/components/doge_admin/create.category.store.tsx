...
import ClientService, { IListAllClientResponse } from './services/client/client.service';
...
export default function CreateCategories() {
    ...
    const [clientList, setClientList] = useState<IListAllClientResponse[]>([]);

    useEffect(() => {
        const fetchData = async () =>{
            const clientService = new ClientService();
            const clientList = await clientService.listAllClient();
    ...
    const RenderCardCategory = ({ name, image_url, index }: ICategory & { index: number }) => {
        return (
        ...
        <div>
            {
                ...
                <div className="max-w-6xl mx-auto p-6 bg-gray-800 rounded-lg shadow-lg">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            <label className="block text-gray-300 font-medium mb-2">Selecione uma Loja</label>
                            <select>
                                {
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                            {categories.map((category, index) => (
                                <RenderCardCategory key={index} image_url={category.image_url} index={index} name={category.name}/>
                            ))}
                        </div>
